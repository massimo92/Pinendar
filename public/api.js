...
export const api = {
  ...
  adminLogin: (username, password) => request('/api/v1/admin/auth/login', json('POST', { username, password })),
  adminLogout: () => request('/api/v1/admin/auth/logout', { method: 'POST' }),
  adminOverview: () => request('/api/v1/admin'),
  approveSignup: (id) => request(`/api/v1/admin/signup-requests/${encodeURIComponent(id)}/approve`, { method: 'POST' }),
  rejectSignup: (id) => request(`/api/v1/admin/signup-requests/${encodeURIComponent(id)}/reject`, { method: 'POST' }),
  createAccount: (body) => request('/api/v1/admin/accounts', json('POST', body)),
  updateAccount: (id, body) => request(`/api/v1/admin/accounts/${encodeURIComponent(id)}`, json('PATCH', body)),
  deleteAccount: (id) => request(`/api/v1/admin/accounts/${encodeURIComponent(id)}`, { method: 'DELETE' }),
  createBackup: () => request('/api/v1/admin/backups', { method: 'POST' }),
  ...
  exchangeAssignments: (id, body) => request(`/api/v1/calendar/events/${encodeURIComponent(id)}/exchange`, json('POST', body)),
  assignmentTransferOptions: (id, includeFixed = false) => request(`/api/v1/calendar/events/${encodeURIComponent(id)}/transfer-options${includeFixed ? '?includeFixed=true' : ''}`),
