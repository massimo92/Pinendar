...
let modal = null;
let holidayMonth = monthKey(dateKey(new Date()));
...
function calendarKpis() {
  ...
  const deferredLabel = summary.deferred === 1
    ? (state.language === 'es' ? 'agenda diferida' : 'agenda diferida')
    : (state.language === 'es' ? 'agendas diferidas' : 'agendes diferides');
  const peonadaLabel = summary.peonada === 1
    ? 'peonada'
    : (state.language === 'es' ? 'peonadas' : 'peonades');
  return `<section class="calendar-kpis" aria-label="${state.language === 'es' ? 'Incidencias del periodo visible' : 'Incidències del període visible'}">
    ...
    <button type="button" class="calendar-kpi deferred ${selectedCalendarIssueFilters.has('deferred') ? 'active' : ''}" data-calendar-issue-filter="deferred" aria-pressed="${selectedCalendarIssueFilters.has('deferred')}"><i aria-hidden="true">D</i><span><b>${summary.deferred}</b><small>${deferredLabel}</small></span></button>
    <button type="button" class="calendar-kpi peonada ${selectedCalendarIssueFilters.has('peonada') ? 'active' : ''}" data-calendar-issue-filter="peonada" aria-pressed="${selectedCalendarIssueFilters.has('peonada')}"><i aria-hidden="true">P</i><span><b>${summary.peonada}</b><small>${peonadaLabel}</small></span></button>
  </section>`;
}
...
function calendarCell(key) {
  ...
  const peonadaItems = events.assignments.filter((item) => item.type !== 'no_assignment' && item.peonada);
  const deferredEvents = deferredItems.map(assignmentEvent);
  const peonadaEvents = peonadaItems.map(assignmentEvent);
  ...
  const allEvents = selectedCalendarIssueFilters.size
    ? uniqueEventEntries([
      ...
      ...(selectedCalendarIssueFilters.has('deferred') ? deferredEvents : []),
      ...(selectedCalendarIssueFilters.has('peonada') ? peonadaEvents : []),
    ])
    : uniqueEventEntries([...issueEvents, ...absenceEvents, ...assignmentEvents]);
...
function holidayCalendar() {
  ...
  const selected = new Set(state.holidays || []);
  const today = dateKey(new Date());
  const first = `${month}-01`; const start = startOfWeek(first); const days = Array.from({ length: 42 }, (_, index) => addDays(start, index));
  const title = fmtDate(first, { month: 'long', year: 'numeric' });
  return `<section class="vacation-calendar" data-holiday-calendar><div class="vacation-calendar-head"><button type="button" class="icon-button" data-action="holiday-prev" aria-label="Mes anterior">‹</button><h3>${title}</h3><button type="button" class="icon-button" data-action="holiday-next" aria-label="Mes següent">›</button></div><div class="vacation-weekdays">${WEEK_SHORT.map((day) => `<span>${day}</span>`).join('')}</div><div class="vacation-days">${days.map((key) => { const chosen = selected.has(key); const locked = key <= today; return `<button type="button" data-action="toggle-holiday" data-holiday-date="${key}" class="${monthKey(key) === month ? '' : 'outside'} ${chosen ? 'selected' : ''} ${locked ? 'past' : ''}" ${locked ? 'disabled' : ''} aria-pressed="${chosen}">${fromKey(key).getUTCDate()}</button>`; }).join('')}</div></section>`;
}
function refreshHolidayCalendar() { const calendar = $('[data-holiday-calendar]'); if (calendar) calendar.outerHTML = holidayCalendar(); }
...
function setupPage() {
  return `${header('Configuració', 'Hospitals coberts i festius')}
    ...
    <section class="section"><div class="card panel holidays-panel"><div class="holidays-panel-head"><div class="card-kicker">CALENDARI</div><h2>Festius</h2></div><div class="holidays-calendar-wrap">${holidayCalendar()}</div></div></section>`;
}
...
function guidePage() {
  ...
  const hardRules = [
    ...
    g('En un dia telemàtic només pot rebre activitat telemàtica; qualsevol agenda presencial fa que el dia sigui presencial.', 'En un día telemático sólo puede recibir actividad telemática; cualquier agenda presencial hace que el día sea presencial.'),
    g('Les regles fixes s’apliquen quan la persona està disponible, però mai no creen una plaça que no existeix.', 'Las reglas fijas se aplican cuando la persona está disponible, pero nunca crean una plaza que no existe.'),
    g('La jornada ordinària és del 100%. Només una peonada indicada expressament pot elevar la càrrega total fins al 200%.', 'La jornada ordinaria es del 100%. Sólo una peonada indicada expresamente puede elevar la carga total hasta el 200%.'),
    g('Una persona només pot tenir una guàrdia en una mateixa data. La postguàrdia del dia natural següent bloqueja qualsevol assignació.', 'Una persona sólo puede tener una guardia en una misma fecha. La postguardia del día natural siguiente bloquea cualquier asignación.'),
  ];
  const preparation = [
    [g('Hospitals', 'Hospitales'), g('Afegeix els centres on es treballa.', 'Añade los centros donde se trabaja.')],
    [g('Agendes', 'Agendas'), g('Indica hospital, torn, càrrega, modalitat, cobertura, prioritat i recurrències especials.', 'Indica hospital, turno, carga, modalidad, cobertura, prioridad y recurrencias especiales.')],
    [g('Equip', 'Equipo'), g('Revisa patrons de treball, capacitats, dies telemàtics, Gestió, vacances i regles fixes.', 'Revisa patrones de trabajo, capacidades, días telemáticos, Gestión, vacaciones y reglas fijas.')],
    [g('Guàrdies', 'Guardias'), g('Pot haver-hi diverses persones de guàrdia el mateix dia. Cada guàrdia crea automàticament la seva postguàrdia.', 'Puede haber varias personas de guardia el mismo día. Cada guardia crea automáticamente su propia postguardia.')],
    [g('Període', 'Período'), g('Genera un mes complet o unes dates dins del mateix mes i tria un màxim d’1 a 30 minuts.', 'Genera un mes completo o unas fechas dentro del mismo mes y elige un máximo de 1 a 30 minutos.')],
  ];
  const generationSteps = [
    [g('Cobertura presencial', 'Cobertura presencial'), g('Cobreix primer les agendes presencials, de prioritat molt alta a baixa.', 'Cubre primero las agendas presenciales, de prioridad muy alta a baja.')],
    [g('Cobertura de Gestió', 'Cobertura de Gestión'), g('Reparteix els dies de Gestió per rondes: intenta donar-ne un a tothom abans de donar-ne un segon.', 'Reparte los días de Gestión por rondas: intenta dar uno a todos antes de dar un segundo.')],
    [g('Cobertura telemàtica', 'Cobertura telemática'), g('Cobreix les agendes telemàtiques per prioritat i intenta recuperar com a diferides les que quedin vacants.', 'Cubre las agendas telemáticas por prioridad e intenta recuperar como diferidas las que queden vacantes.')],
    [g('Equilibri operatiu', 'Equilibrio operativo'), g('Redueix jornades incompletes, equilibra el teletreball i les agendes segons l’històric, i evita repeticions consecutives. En dies de guàrdia prefereix almenys una agenda presencial.', 'Reduce jornadas incompletas, equilibra el teletrabajo y las agendas según el histórico, y evita repeticiones consecutivas. En días de guardia prefiere al menos una agenda presencial.')],
    [g('Poliment final', 'Pulido final'), g('Prova intercanvis segurs per millorar encara més l’equitat i el percentatge telemàtic sense perdre cobertura.', 'Prueba intercambios seguros para mejorar aún más la equidad y el porcentaje telemático sin perder cobertura.')],
  ];
  return `<section class="guide-page">
    ...
    <section class="guide-hero card">
      <div><div class="eyebrow">${g('IDEA PRINCIPAL', 'IDEA PRINCIPAL')}</div><h2>${g('Pinendar proposa. Tu decideixes.', 'Pinendar propone. Tú decides.')}</h2><p>${g('La plataforma prepara el calendari, assenyala els problemes i permet revisar o canviar el resultat. Si una generació falla, el calendari actual no es modifica.', 'La plataforma prepara el calendario, señala los problemas y permite revisar o cambiar el resultado. Si una generación falla, el calendario actual no se modifica.')}</p></div>
      <div class="guide-quick"><b>${g('En 30 segons', 'En 30 segundos')}</b><span>1. ${g('Actualitza dades i condicionants', 'Actualiza datos y condicionantes')}</span><span>2. ${g('Genera el període', 'Genera el período')}</span><span>3. ${g('Revisa avisos i mètriques', 'Revisa avisos y métricas')}</span><span>4. ${g('Ajusta i exporta', 'Ajusta y exporta')}</span></div>
    </section>
    ...
    <section class="guide-section card"><div class="guide-section-head"><span>2</span><div><h2>${g('Regles que mai no es trenquen', 'Reglas que nunca se rompen')}</h2></div></div><ul class="guide-check-list">${hardRules.map((rule) => `<li><span>✓</span>${rule}</li>`).join('')}</ul></section>

    <section class="guide-section card"><div class="guide-section-head"><span>3</span><div><h2>${g('Com tria el millor repartiment', 'Cómo elige el mejor reparto')}</h2><p>${g('Segueix aquest ordre. Cada pas conserva el millor resultat aconseguit en els anteriors.', 'Sigue este orden. Cada paso conserva el mejor resultado conseguido en los anteriores.')}</p></div></div>
      <ol class="guide-priority-list">${generationSteps.map(([title, description]) => `<li><b>${title}</b><span>${description}</span></li>`).join('')}</ol>
      <div class="guide-note"><b>${g('Temps màxim:', 'Tiempo máximo:')}</b> ${g('pots triar entre 1 i 30 minuts. La generació pot acabar abans si demostra que ja no hi ha una solució millor. Si arriba al límit, conserva la millor solució vàlida trobada.', 'puedes elegir entre 1 y 30 minutos. La generación puede terminar antes si demuestra que ya no hay una solución mejor. Si alcanza el límite, conserva la mejor solución válida encontrada.')}</div>
    ...
    <section class="guide-section card"><div class="guide-section-head"><span>4</span><div><h2>${g('Com llegir el calendari', 'Cómo leer el calendario')}</h2><p>${g('Els avisos i filtres del període ajuden a trobar ràpidament què cal revisar.', 'Los avisos y filtros del período ayudan a encontrar rápidamente qué hay que revisar.')}</p></div></div><div class="guide-signal-grid">
      ...
      <article class="deferred"><i>D</i><div><b>${g('Agenda diferida', 'Agenda diferida')}</b><p>${g('És una vacant telemàtica feta més tard. El detall conserva la data original.', 'Es una vacante telemática realizada más tarde. El detalle conserva la fecha original.')}</p></div></article>
      <article class="peonada"><i>P</i><div><b>${g('Peonada', 'Peonada')}</b><p>${g('És càrrega extraordinària per sobre del 100% ordinari.', 'Es carga extraordinaria por encima del 100% ordinario.')}</p></div></article>
      <article class="partial"><i>~</i><div><b>${g('Jornada parcial', 'Jornada parcial')}</b><p>${g('La persona només té una agenda del 50% aquell dia.', 'La persona sólo tiene una agenda del 50% ese día.')}</p></div></article>
    </div><p class="guide-footnote">${g('Pots combinar els filtres de persona i agenda amb els d’agendes sense cobrir, dies-persona sense agenda, diferits i peonades. En dia i setmana, les targetes s’agrupen per hospital; en mes, el color identifica l’agenda.', 'Puedes combinar los filtros de persona y agenda con los de agendas sin cubrir, días-persona sin agenda, diferidos y peonadas. En día y semana, las tarjetas se agrupan por hospital; en mes, el color identifica la agenda.')}</p></section>
    ...
    <section class="guide-section card"><div class="guide-section-head"><span>5</span><div><h2>${g('Canvis després de generar', 'Cambios después de generar')}</h2><p>${g('El calendari continua sent editable, però cada acció té un significat diferent.', 'El calendario sigue siendo editable, pero cada acción tiene un significado diferente.')}</p></div></div><div class="guide-action-grid">
      ...
      <article><b>${g('Diferir una agenda', 'Diferir una agenda')}</b><p>${g('Una vacant telemàtica es pot fer dins dels sis dies naturals següents. També pots començar des d’una persona sense agenda i adoptar una vacant anterior compatible.', 'Una vacante telemática puede realizarse dentro de los seis días naturales siguientes. También puedes empezar desde una persona sin agenda y adoptar una vacante anterior compatible.')}</p></article>
    ...
    </div><div class="guide-note"><b>${g('Important:', 'Importante:')}</b> ${g('els canvis manuals queden bloquejats i es conserven en regenerar el període. Si una operació incompleix una regla dura, no s’aplica res.', 'los cambios manuales quedan bloqueados y se conservan al regenerar el período. Si una operación incumple una regla dura, no se aplica nada.')}</div></section>

    <section class="guide-section card"><div class="guide-section-head"><span>6</span><div><h2>${g('Regles fixes', 'Reglas fijas')}</h2><p>${g('Defineixen allò que una persona ha de fer de manera recurrent quan està disponible.', 'Definen lo que una persona debe hacer de forma recurrente cuando está disponible.')}</p></div></div><div class="guide-action-grid">
      <article><b>${g('Ha de fer totes', 'Debe hacer todas')}</b><p>${g('Assigna totes les agendes seleccionades que tinguin demanda aquell dia. Si superen el 100%, cal marcar exactament quines es faran com a peonada.', 'Asigna todas las agendas seleccionadas que tengan demanda ese día. Si superan el 100%, hay que marcar exactamente cuáles se harán como peonada.')}</p></article>
      <article><b>${g('Ha de fer una', 'Debe hacer una')}</b><p>${g('Assigna exactament una de les agendes seleccionades que tinguin demanda.', 'Asigna exactamente una de las agendas seleccionadas que tengan demanda.')}</p></article>
      <article><b>${g('No pot fer', 'No puede hacer')}</b><p>${g('Impedeix les agendes seleccionades aquell dia de la setmana.', 'Impide las agendas seleccionadas ese día de la semana.')}</p></article>
      <article><b>${g('Si hi ha un conflicte', 'Si hay un conflicto')}</b><p>${g('La regla no es compleix a mitges: la generació falla i el calendari vigent es conserva. Revisa demanda, capacitats, absències i altres assignacions bloquejades.', 'La regla no se cumple a medias: la generación falla y el calendario vigente se conserva. Revisa demanda, capacidades, ausencias y otras asignaciones bloqueadas.')}</p></article>
    </div></section>

    <section class="guide-section card"><div class="guide-section-head"><span>7</span><div><h2>${g('Històric, mètriques i exportació', 'Histórico, métricas y exportación')}</h2><p>${g('Cada període s’integra al mateix calendari i els períodes anteriors es conserven.', 'Cada período se integra en el mismo calendario y los períodos anteriores se conservan.')}</p></div></div><div class="guide-action-grid">
    ...
    <section class="guide-section guide-faq card"><div class="guide-section-head"><span>?</span><div><h2>${g('Preguntes freqüents', 'Preguntas frecuentes')}</h2></div></div>
      ...
      <details><summary>${g('Per què la generació pot acabar abans del temps escollit?', '¿Por qué la generación puede terminar antes del tiempo elegido?')}</summary><p>${g('El temps és un màxim, no una durada obligatòria. Si Pinendar demostra que ja no hi ha una solució millor, acaba abans.', 'El tiempo es un máximo, no una duración obligatoria. Si Pinendar demuestra que ya no hay una solución mejor, termina antes.')}</p></details>
      <details><summary>${g('Què passa si torno a generar un període existent?', '¿Qué pasa si vuelvo a generar un período existente?')}</summary><p>${g('Pinendar demana confirmació i substitueix només aquell interval. Conserva guàrdies, absències i canvis manuals bloquejats. Si la nova generació falla, no canvia res.', 'Pinendar pide confirmación y sustituye sólo ese intervalo. Conserva guardias, ausencias y cambios manuales bloqueados. Si la nueva generación falla, no cambia nada.')}</p></details>
      <details><summary>${g('Quan es pot diferir una agenda?', '¿Cuándo se puede diferir una agenda?')}</summary><p>${g('Només quan és telemàtica, ha quedat vacant i una persona capacitada té càrrega lliure dins dels sis dies naturals següents. En un dia de guàrdia, Pinendar protegeix abans que hi hagi almenys una agenda presencial sempre que sigui possible.', 'Sólo cuando es telemática, ha quedado vacante y una persona capacitada tiene carga libre dentro de los seis días naturales siguientes. En un día de guardia, Pinendar protege antes que haya al menos una agenda presencial siempre que sea posible.')}</p></details>
...
function existingGenerationCondition(item, kind) {
  const member = person(item.memberId);
  const label = kind === 'guard' ? `Guàrdia · ${item.date}` : `Vacances · ${item.start}${item.end !== item.start ? ` — ${item.end}` : ''}`;
  return `<div class="generation-condition existing"><span><b>${esc(member?.name || '—')}</b><small>${esc(label)}</small></span></div>`;
}
...
function generationMissingConditions(bounds = generationDateBounds()) {
  const hasHoliday = state.holidays.some((date) => date >= bounds.start && date <= bounds.end);
  const hasGuard = [...calendarGuards(), ...(modal.guards || [])].some((item) => item.date >= bounds.start && item.date <= bounds.end);
  const hasVacation = [...calendarAbsences(), ...(modal.absences || [])].some((item) => item.category === 'vacances' && item.start <= bounds.end && item.end >= bounds.start);
  return [!hasHoliday && 'festius', !hasGuard && 'guàrdies', !hasVacation && 'vacances'].filter(Boolean);
}
...
function startGenerationLoadingAnimation() {
  ...
  const countdownTimer = window.setInterval(updateGenerationCountdown, 250);
  return [phraseTimer, countdownTimer];
}

function generationCountdownText() {
  const es = state.language === 'es';
  const duration = Number(modal?.loadingTimeLimitSeconds || 0);
  if (!duration) return es ? 'Calculando el tiempo máximo…' : 'Calculant el temps màxim…';
  const elapsed = Math.max(0, (Date.now() - Number(modal.loadingStartedAt || Date.now())) / 1000);
  const remaining = Math.max(0, Math.ceil(duration - elapsed));
  if (!remaining) return es ? 'Finalizando la mejor solución encontrada…' : 'Finalitzant la millor solució trobada…';
  const minutes = Math.floor(remaining / 60);
  const seconds = String(remaining % 60).padStart(2, '0');
  return `${es ? 'Tiempo máximo restante' : 'Temps màxim restant'} · ${minutes}:${seconds}`;
}

function updateGenerationCountdown() {
  if (!modal?.busy) return;
  const countdown = document.querySelector('[data-generation-countdown]');
  if (countdown) countdown.textContent = generationCountdownText();
}

function generationLoadingModal() {
  const es = state.language === 'es';
  return `<div class="modal-backdrop"><section class="modal-card modal-small generation-loading-modal" role="dialog" aria-modal="true" aria-labelledby="generation-loading-title" aria-describedby="generation-loading-help"><div class="generation-loading-body"><div class="generation-loading-orbit" aria-hidden="true"><i></i><i></i><i></i></div><div class="card-kicker">${es ? 'OPTIMIZANDO' : 'OPTIMITZANT'}</div><h2 id="generation-loading-title">${es ? 'Preparando el calendario' : 'Preparant el calendari'}</h2><p data-generation-loading-message aria-live="polite">${esc(modal.loadingPhrase)}</p><div class="generation-loading-track" aria-hidden="true"><i></i></div><div class="generation-loading-countdown" data-generation-countdown aria-live="off">${generationCountdownText()}</div><small id="generation-loading-help">${es ? 'Puede tardar unos minutos. No cierres esta ventana.' : 'Pot trigar uns minuts. No tanquis aquesta finestra.'}</small></div></section></div>`;
}
...
function generationModal() {
  ...
  const bounds = generationDateBounds();
  const existingGuards = calendarGuards().filter((item) => item.date >= bounds.start && item.date <= bounds.end).map((item) => existingGenerationCondition(item, 'guard')).join('');
  const existingVacations = calendarAbsences().filter((item) => item.category === 'vacances' && item.start <= bounds.end && item.end >= bounds.start).map((item) => existingGenerationCondition(item, 'absence')).join('');
...
function generationConditionsWarningModal() {
  return `<div class="modal-backdrop"><section class="modal-card modal-small generation-conditions-warning-modal" role="alertdialog" aria-modal="true" aria-labelledby="conditions-warning-title"><div class="modal-body"><div class="generation-warning-icon" aria-hidden="true">⚠</div><div class="card-kicker">REVISA ELS CONDICIONANTS</div><h2 id="conditions-warning-title">Falten condicionants</h2><p>No s'han detectat ${modal.missing.join(', ')} en aquest període.</p></div><div class="modal-actions"><button type="button" class="button ghost" data-action="return-generation-from-warning">Torna enrere</button><button type="button" class="button warning" data-action="confirm-generation-conditions">Genera igualment</button></div></section></div>`;
}
function exportModal() {
  const label = { csv: 'CSV', excel: 'Excel', ics: 'ICS' }[modal.format] || 'fitxer';
  return `<div class="modal-backdrop" data-action="close-modal"><section class="modal-card modal-small" role="dialog" aria-modal="true" aria-labelledby="export-title"><div class="modal-head"><div><div class="card-kicker">EXPORTACIÓ</div><h2 id="export-title">Exporta ${label}</h2><div class="muted">Escull el rang de dates a exportar.</div></div><button class="icon-button" data-action="close-modal" aria-label="Tanca">×</button></div><form id="export-form"><div class="modal-body"><div class="form-grid"><div class="field"><label>Data inicial</label><input type="date" name="startDate" value="${modal.startDate}" required /></div><div class="field"><label>Data final</label><input type="date" name="endDate" value="${modal.endDate}" min="${modal.startDate}" required /></div></div></div><div class="modal-actions"><button type="button" class="button ghost" data-action="close-modal">Cancel·la</button><button type="button" class="button" data-action="submit-modal">Exporta ${label}</button></div></form></section></div>`;
}
function fixedAssignmentWarningModal() {
  ...
  return `<div class="modal-backdrop" data-action="close-modal"><section class="modal-card modal-small modal-fixed-warning" role="alertdialog" aria-modal="true"><div class="modal-head"><div><div class="card-kicker">ASSIGNACIÓ FIXA</div><h2>Vols canviar-la?</h2></div><button class="icon-button" data-action="close-modal" aria-label="Tanca">×</button></div><div class="modal-body"><div class="fixed-assignment-warning"><span class="fixed-warning-icon" aria-hidden="true">!</span><div><b>${esc(member?.name || '—')} · ${esc(itemAgenda?.name || '—')}${deferredOriginDetail(item)}</b><span>Aquesta assignació prové d’una regla fixa. Si continues, el canvi només afectarà aquest dia; la regla recurrent del perfil es conservarà.</span></div></div></div><div class="modal-actions"><button type="button" class="button ghost" data-action="close-modal">Cancel·la</button><button type="button" class="button" data-action="confirm-fixed-exchange">Sí, continua</button></div></section></div>`;
}

function deferredOriginDetail(item) {
  if (!item?.deferredOriginDate) return '';
  const label = state.language === 'es' ? 'Diferida del' : 'Diferida del';
  return `<span class="assignment-origin-detail">· ${label} ${esc(fmtDate(item.deferredOriginDate, { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' }))}</span>`;
}
...
function assignmentModal() {
  ...
  if (modal.type === 'assignment-exchange') {
    ...
    return `<div class="modal-backdrop" data-action="close-modal"><section class="modal-card modal-assignment-action" role="dialog" aria-modal="true"><div class="modal-head"><div><div class="card-kicker">INTERCANVI D’ASSIGNACIONS</div><h2>${esc(sourceMember?.name || '—')}</h2><div class="muted">${fmtDate(source?.date, { weekday: 'long', day: 'numeric', month: 'long' })} · ${esc(sourceAgenda?.name || '—')}${deferredOriginDetail(source)}</div></div><button class="icon-button" data-action="close-modal">×</button></div><form id="assignment-exchange-form"><input type="hidden" name="id" value="${esc(source?.id || '')}" /><input type="hidden" name="confirmFixed" value="${modal.confirmFixed ? 'true' : 'false'}" /><div class="modal-body"><p class="assignment-action-help">Tria una assignació compatible o una agenda sense cobrir. Pinendar mostrarà l’impacte en l’equitat.</p><div class="assignment-choice-list">${rows || '<div class="assignment-choice-empty">No hi ha canvis compatibles per a aquesta assignació.</div>'}</div>${peonadaAction}</div><div class="modal-actions"><button type="button" class="button ghost" data-action="${modal.returnModal ? 'return-assignment-action' : 'close-modal'}">Cancel·la</button><button type="button" class="button" data-action="submit-modal" ${options.length ? '' : 'disabled'}>Aplica el canvi</button></div></form></section></div>`;
  }
  ...
  const deferredRows = (modal.payload?.deferredOptions || []).map((option) => {
    const item = agenda(option.agendaId);
    const hospital = agendaHospital(item);
    const loadPercentage = Number(option.loadPercentage ?? item?.loadPercentage ?? 100);
    return `<button type="button" class="deferred-option" data-action="apply-member-deferred" data-vacancy-id="${esc(option.vacancyId)}" aria-label="${esc(`${item?.name || '—'} · Diferida del ${fmtDate(option.originDate, { weekday: 'long', day: 'numeric', month: 'long' })}`)}"><span class="assignment-choice-head"><span class="deferred-option-title"><b>${esc(item?.name || '—')}</b><small>· Diferida del ${fmtDate(option.originDate, { weekday: 'long', day: 'numeric', month: 'long' })}</small></span>${fairnessBadge(option)}</span><small>${esc(hospital?.name || 'Sense hospital')} · ${esc(activityMetaTitle(item))} · ${loadPercentage}% de càrrega</small></button>`;
  }).join('');
  const deferredSection = deferredRows
    ? `<section class="vacancy-resolution-section"><h3>Assigna una agenda diferida</h3><p class="assignment-action-help">Pots adoptar directament una agenda dels sis dies anteriors. No es mourà ni s’afegirà cap altra activitat.</p><div class="deferred-option-list">${deferredRows}</div></section>`
    : '';
...
function vacancyAssignmentModal() {
  ...
  const deferredRows = deferredOptions.map((option, index) => {
    ...
    const movementLabels = (option.movements || []).map((movement) => {
      ...
      return `${movedAgenda?.name || '—'}: ${person(movement.fromMemberId)?.name || '—'} → ${person(movement.toMemberId)?.name || '—'}`;
    });
    const movements = movementLabels.length
      ? `<span class="deferred-option-movement-count">${movementLabels.length} ${movementLabels.length === 1 ? 'moviment necessari' : 'moviments necessaris'}</span><ul class="deferred-option-movement-list">${movementLabels.map((label) => `<li>${esc(label)}</li>`).join('')}</ul>`
      : '<span class="deferred-option-movements">No cal moure cap altra assignació.</span>';
    const targetDateLabel = fmtDate(option.targetDate, { weekday: 'long', day: 'numeric', month: 'long' });
...
function modalView() {
  ...
  if (modal.type === 'generation-conditions-warning') return generationConditionsWarningModal();
...
function exportRows(bounds) {
  const withinBounds = (item) => item.date >= bounds.start && item.date <= bounds.end;
  const assigned = calendarEvents().filter((item) => withinBounds(item) && (!selectedMemberFilters.size || selectedMemberFilters.has(item.memberId)) && (!selectedAgendaFilters.size || selectedAgendaFilters.has(item.type)));
  const vacancies = selectedMemberFilters.size ? [] : calendarVacancies()
    .filter((item) => withinBounds(item) && (!selectedAgendaFilters.size || selectedAgendaFilters.has(item.type)))
    .map((item) => ({ ...item, empty: true }));
  return [...assigned, ...vacancies].sort((left, right) => left.date.localeCompare(right.date) || String(left.id).localeCompare(String(right.id)));
}
function exportName(bounds) { return `pinendar-${bounds.start}-${bounds.end}`; }
function exportData(bounds) { return exportRows(bounds).map((item) => { const member = person(item.memberId) || {}; const agendaItem = agenda(item.type); return { Data: item.date, Metge: member.name || '', Correu: member.email || '', Agenda: item.type === 'no_assignment' ? 'NO ASSIGNACIÓ' : agendaItem.name, Hospital: item.type === 'no_assignment' ? '' : agendaHospital(agendaItem)?.name || '', Estat: item.empty ? 'SENSE ASSIGNAR' : 'Assignada', Peonada: item.peonada ? 'Sí' : 'No', Diferida: item.deferredOriginDate ? 'Sí' : 'No', 'Data origen': item.deferredOriginDate || '' }; }); }
function exportCsv(bounds) { const rows = [['Data', 'Metge', 'Correu', 'Agenda', 'Hospital', 'Estat', 'Peonada', 'Diferida', 'Data origen'], ...exportData(bounds).map((item) => Object.values(item))]; download(`${exportName(bounds)}.csv`, `\ufeff${rows.map((row) => row.map((value) => `"${String(value).replaceAll('"', '""')}"`).join(';')).join('\n')}`, 'text/csv;charset=utf-8'); }
function exportIcs(bounds) {
  const events = exportRows(bounds).filter((item) => item.type !== 'no_assignment').map((item) => {
  ...
  download(`${exportName(bounds)}.ics`, buildIcsCalendar(events), 'text/calendar;charset=utf-8');
}
function exportExcel(bounds) { if (!window.XLSX) return toast('No s’ha pogut carregar l’exportador Excel'); const sheet = window.XLSX.utils.json_to_sheet(exportData(bounds)); const workbook = window.XLSX.utils.book_new(); window.XLSX.utils.book_append_sheet(workbook, sheet, 'Pinendar'); const bytes = window.XLSX.write(workbook, { bookType: 'xlsx', type: 'array' }); download(`${exportName(bounds)}.xlsx`, bytes, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); }
...
document.addEventListener('click', async (event) => {
  ...
  if (button.dataset.removeGenerationCondition) { const [kind, id] = button.dataset.removeGenerationCondition.split(':'); if (kind === 'guard') modal.guards = modal.guards.filter((item) => item.id !== id); else modal.absences = modal.absences.filter((item) => item.id !== id); modal.conditionsConfirmed = false; modal.conditionsWarning = false; render(); return; }
  ...
  if (action === 'toggle-holiday') { const date = button.dataset.holidayDate; if (!date || date <= dateKey(new Date())) return; try { if (state.holidays.includes(date)) await api.deleteHoliday(date); else await api.addHoliday(date); if (await reloadState('Configuració desada')) refreshHolidayCalendar(); } catch (error) { showError(error); } return; }
  ...
  if (action === 'confirm-generation-conditions') { modal = modal.previous; modal.conditionsConfirmed = true; render(); await handleForm($('#generation-form')); return; }
  if (action === 'return-generation-from-warning') { modal = modal.previous; render(); return; }
  ...
  if (action === 'open-export') { const bounds = calendarBounds(); modal = { type: 'export', format: button.dataset.exportFormat, startDate: bounds.start, endDate: bounds.end }; render(); return; }
  if (action === 'open-generation') { const startMonth = nextGenerationMonth(); const startDate = `${startMonth}-01`; modal = { type: 'generation', periodMode: 'month', startMonth, endMonth: startMonth, startDate, endDate: endOfMonth(startDate), timeLimitMinutes: 2, guards: [], absences: [], conflict: '' }; render(); return; }
  ...
  if (action === 'backup') download(`pinendar-backup-${dateKey(new Date())}.json`, JSON.stringify(state, null, 2), 'application/json');
});
...
document.addEventListener('change', async (event) => {
  ...
  if (modal?.type === 'generation' && event.target.name === 'periodMode') { modal.periodMode = event.target.value; modal.replaceExisting = false; modal.conditionsConfirmed = false; modal.conditionsWarning = false; modal.overlap = null; modal.conflict = ''; render(); return; }
...
async function handleForm(formElement) {
  ...
  try {
    ...
    if (formId === 'generation-form') {
      ...
      const missingConditions = generationMissingConditions(bounds);
      if (missingConditions.length && !modal.conditionsConfirmed) { modal = { type: 'generation-conditions-warning', previous: modal, missing: missingConditions }; render(); return; }
      modal.busy = true; modal.conflict = ''; modal.loadingTimeLimitSeconds = null; modal.loadingStartedAt = Date.now(); modal.loadingPhrase = nextGenerationLoadingMessage(); render();
      const loadingTimers = startGenerationLoadingAnimation();
      let job;
      try {
        const queued = await api.startGeneration({ startMonth, endMonth, startDate, endDate, timeLimitMinutes: Number(form.get('generationTimeLimitMinutes')), guards: modal.guards, absences: modal.absences, replaceExisting: Boolean(modal.replaceExisting) });
        modal.loadingTimeLimitSeconds = Number(queued.timeLimitSeconds || 120); modal.loadingStartedAt = Date.now(); updateGenerationCountdown();
        job = await waitForGeneration(queued.id);
      } finally {
        loadingTimers.forEach((timer) => window.clearInterval(timer));
      }
    ...
    if (formId === 'export-form') { const bounds = { start: form.get('startDate'), end: form.get('endDate') }; if (bounds.end < bounds.start) return toast('La data final no pot ser anterior a la data inicial'); const format = modal.format; if (format === 'csv') exportCsv(bounds); if (format === 'excel') exportExcel(bounds); if (format === 'ics') exportIcs(bounds); modal = null; render(); }
  } catch (error) {
    if (formId === 'generation-form') {
      modal.busy = false;
...
document.addEventListener('submit', async (event) => {
  ...
  if (!['generation-form', 'member-form', 'delete-member-form', 'agenda-form', 'assignment-exchange-form', 'assignment-transfer-form', 'vacancy-assignment-form', 'peonada-review-form', 'extra-assignment-form', 'hospital-form', 'export-form', 'clear-calendar-form'].includes(event.target.getAttribute('id'))) return;
  event.preventDefault(); await handleForm(event.target);
}, true);

document.addEventListener('input', (event) => {
  if (modal?.type === 'generation' && event.target.name === 'generationTimeLimitMinutes') {
    const minutes = clampGenerationTimeLimit(event.target.value);
    if (minutes === null) return;
    event.target.value = String(minutes);
    modal.timeLimitMinutes = minutes;
    return;
  }
